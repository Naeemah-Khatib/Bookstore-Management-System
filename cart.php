...
<?php
        ...
        if(isset($_GET['remove']))
                 {  $product=$_GET['remove'];
                    $query="DELETE FROM cart where username='$customer' AND isbn='$product'";
                    $result=mysqli_query($con,$query);                   
                   ?>
                    <script type="text/javascript">
  ...
  <nav class="navbar navbar-default navbar-fixed-top navbar-inverse">
    <div class="container-fluid">
        <div class="navbar-header">
          	...
          	<a class="navbar-brand" href="index.php" style="padding: 1px;"><img class="img-responsive" alt="Brand" src="img/logo.png"></a>
        </div>
	                                    ...
	                                    <div class="col-md-12 text-center">
                                            <form class="form" role="form" method="post" action="index.php" accept-charset="UTF-8">
                                                <div class="form-group">
	                                                ...
	                                                <input type="text" name="register_fname" class="form-control" placeholder="Full Name" required>
	                                            </div>
	                                            <div class="form-group">
	                                                <label class="sr-only" for="username">Username</label>
                                                ...
                                                <div class="form-group">
	                                                <label class="sr-only" for="phno">PhoneNumber</label>
	                                                <input type="text" name="register_phno" class="form-control" placeholder="Phone Number" required>
	                                            </div>
	                                            <div class="form-group">
	...
	if(isset($_SESSION['user']))
	    {   
	            ...
	            {   
                        ...
                        $query="SELECT * from cart where username='$customer' AND isbn='$product'";
                        $result=mysqli_query($con,$query);
                        $row = mysqli_fetch_assoc($result);
                        if(mysqli_num_rows($result)==0)
	                         {
                              $query="INSERT INTO cart values('$customer','$product','$quantity1')"; 
                              $result=mysqli_query($con,$query);
                              date_default_timezone_set('Asia/Kolkata');
                              $x=date("Y-m-d h:i:sa");
                              $new=$_GET['quantity'];
                              $y=0;
                              $query="INSERT into orders values('$x','$new','$y')";
                              $result=mysqli_query($con,$query); 

                              mail("naeemah89khatib@gmail.com","A new order","Username: '$customer' \nISBN: '$product' \nQauntity: '$new' \nDate: '$x");
                            }
                        else
                           { 
                             $new=$_GET['quantity'];
                             $query="UPDATE `cart` SET quantity=$new WHERE username='$customer' AND isbn='$product'";
                             $result=mysqli_query($con,$query);
                             
                            }
                    }
              	$query="SELECT isbn,title,bauthor,quantity,price FROM cart NATURAL JOIN book 
                                      ...
                                      <div class="panel col-xs-12 col-sm-4 col-sm-offset-'.$offset.' col-md-4 col-md-offset-'.$offset.' col-lg-4 col-lg-offset-'.$offset.' text-center" style="color:#D67B22;font-weight:800;">
                                          ...
                                          <div class="panel-body">
			                                                <img class="image-responsive block-center" src="'.$path.'" style="height :100px;"> <br>
           							                                               title : '.$row['title'].'  <br> 
                                                                        Code : '.$row['isbn'].'     <br>        	 
                                                      									Author : '.$row['bauthor'].' <br>                            	      
                                                         								Quantity : '.$row['quantity'].' <br>
                                                      									Price : '.$row['price'].' <br>
                                                      									Sub Total : '.$Stotal.' <br>
                                                                       <a href="cart.php?remove='.$row['isbn'].'" class="btn btn-sm" 
